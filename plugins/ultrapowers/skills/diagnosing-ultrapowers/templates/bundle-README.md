# Ultrapowers session diagnosis bundle

Session: <session-id>
Harness: <name> <version> (<provenance label>)    Ultrapowers: <version> (<sha or "not a checkout">; <provenance label>)
...
A scrubbed record of a coding-agent session that had ultrapowers installed
and went wrong. It lets an agent or person who was not present decide
whether ultrapowers contributed and, if so, what to change. The report
inside states what happened with `path:line` evidence. By design it
contains no diagnosis of ultrapowers and no proposed fix; that is the
reader's job.
...
Reconcile report, case, environment, findings and README. Refresh scrub-log counts against final files excluding the log itself.
