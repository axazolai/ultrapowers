...
2. Using the skill-invocation and attribution meanings established in the case
   ...
   happened in.
3. List every non-ultrapowers plugin, skill, agent type, MCP server, or
   ...
   with something other than `ultrapowers`.
4. For each human turn, compare the request text against the trigger
   descriptions of the ultrapowers skills installed (read
   ...
   - each non-ultrapowers plugin/skill/tool used, with where.
